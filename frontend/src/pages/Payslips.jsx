  ...
  return (
    <div>
      ...
      <div className="card"><div className="tableWrap"><table>
        <thead><tr><th>Employee</th><th>Pay Run</th><th>Period</th><th>Status</th><th>Worked Days</th><th>Net</th><th></th></tr></thead>
        <tbody>{DB.payslips.map(ps=>{
          ...
          return <tr key={ps.id} className="clickable" onClick={()=>nav(`/payslips/${ps.id}`)}>
            <td>{emp?.name}</td><td>{pr?.name}</td><td>{periodLabel(ps.period)}</td><td><span className={`pill ${ps.status==='Paid'?'green':ps.status==='Computed'?'blue':'amber'}`}>{ps.status}</span></td><td>{ps.workedDays||22}</td><td><b>{fmtMoney(ps.net)}</b></td><td style={{textAlign:'right'}}><button className="btn small" onClick={event=>{event.stopPropagation(); window.open(api.payslipPdfUrl(ps.id),'_blank','noopener');}}>Download PDF</button></td>
          </tr>;
        })}</tbody>
      </table></div></div>
    </div>
